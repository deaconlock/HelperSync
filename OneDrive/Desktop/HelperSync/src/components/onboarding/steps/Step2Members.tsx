...
import { HouseholdMember, MemberRole, TimePreset } from "@/types/household";
...
const TIME_PRESETS: { value: TimePreset; label: string; emoji: string }[] = [
  { value: "morning",   label: "Mornings",   emoji: "☀️" },
  { value: "afternoon", label: "Afternoons", emoji: "🌤️" },
  { value: "evening",   label: "Evenings",   emoji: "🌙" },
  { value: "allday",    label: "All-day",    emoji: "🏠" },
];

function timePresetToText(presets: TimePreset[]): string {
  if (!presets || presets.length === 0) return "";
  const labels = presets.map((p) => TIME_PRESETS.find((t) => t.value === p)?.label ?? p);
  return `Usually home: ${labels.join(", ")}`;
}
...
export function Step2Members({ members, setupFor, onUpdate }: Step2Props) {
  ...
  const toggleTimePreset = (index: number, preset: TimePreset) => {
    const member = localMembers[index];
    const current = member.timePresets ?? [];
    let next: TimePreset[];
    if (current.includes(preset)) {
      next = current.filter((p) => p !== preset);
    } else if (preset === "allday") {
      // All-day clears the specific time slots
      next = ["allday"];
    } else {
      // Selecting a specific slot clears all-day
      next = [...current.filter((p) => p !== "allday"), preset];
    }
    updateMember(index, { timePresets: next });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {localMembers.map((member, i) => {
          ...
          const presets = member.timePresets ?? [];
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-1">
                    ...
                    {presets.map((p) => {
                      const t = TIME_PRESETS.find((x) => x.value === p);
                      return t ? (
                        <span key={p} className="text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">
                          {t.emoji} {t.label}
                        </span>
                      ) : null;
                    })}
                ...
                <div className="border-t border-border px-4 py-4 space-y-5">
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-500 mb-0.5">
                      {member.role === "Me" ? "When are you usually home?" : member.role === "Pets" ? "When does your pet need attention?" : "When are they usually home?"}
                    </label>
                    <p className="text-xs text-gray-400 mb-2">Think typical week — no need to be exact.</p>
                    <div className="flex flex-wrap gap-2">
                      {TIME_PRESETS.map((t) => {
                        const isSelected = presets.includes(t.value);
                        const isAlldayActive = presets.includes("allday");
                        const isGreyed = isAlldayActive && t.value !== "allday";
                        return (
                          <button
                            key={t.value}
                            onClick={() => toggleTimePreset(i, t.value)}
                            disabled={isGreyed}
                            className={cn(
                              "flex items-center gap-1.5 px-3 py-2 rounded-xl border text-sm font-medium transition-all duration-150",
                              isSelected

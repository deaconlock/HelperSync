...
import { Step3Refinement } from "@/components/onboarding/steps/Step3Refinement";
...
function EmployerOnboardingPage() {
    ...
    try {
      const res = await fetch("/api/ai/generate-timetable", {
        ...
        body: JSON.stringify({
          ...
          routines: [d.householdRoutine, d.routineRefinement, d.servicePrefsRoutine].filter(Boolean).join("\n"),
          memberRoutines: {},
          memberQuietHours: {},
  ...
  return (
    <WizardShell
      ...
      wide={step === 7}
      hideFooter={step === 3 || step === 7 || step === 8 || showStep4Reward}
    >
      {step === 1 && (
        <Step1Household
          rooms={data.rooms}
      ...
      {step === 3.5 && (
        <Step4bServicePrefs
          members={data.members}
          householdFocus={data.householdFocus}
          onComplete={(servicePrefsRoutine) => {
            updateData({ servicePrefsRoutine });
            goToStep(1.5);
          }}
        />
      )}

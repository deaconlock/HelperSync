...
import { ChevronDown, ChevronUp, ChevronRight, ChevronLeft, Loader2, Lock, Zap, Settings2 } from "lucide-react";
import { cn } from "@/lib/utils";
import { HouseholdMember } from "@/types/household";
import { HouseholdRule } from "./HouseholdRulesSheet";
...
interface Props {
  ...
  rules?: HouseholdRule[];
  appliedRulesSnapshot?: { id: string; title: string }[];
  onManageRules?: () => void;
}
...
export function QuickRefineSheet({ open, onClose, onRegenerate, household, rules = [], appliedRulesSnapshot = [], onManageRules }: Props) {
  const [view, setView] = useState<"hub" | "preferences">("hub");
  ...
  return (
    <BottomSheet open={open} onClose={handleClose} className="max-h-[85vh] overflow-y-auto">
      <div className="pb-24">
        ...
        {view === "hub" && (
          <>
            <p className="text-sm font-semibold text-gray-900 mb-0.5">Quick Refine</p>
            <p className="text-xs text-gray-400 mb-4">What would you like to adjust?</p>

            <div className="flex flex-col gap-2">
              {/* Household Rules */}
              <button
                onClick={onManageRules}
                className="w-full flex items-center justify-between px-4 py-3.5 rounded-xl bg-gray-50 border border-gray-100 hover:bg-gray-100 transition-colors"
              >
                <span className="flex items-center gap-3">
                  <span className="w-8 h-8 rounded-lg bg-gray-900 flex items-center justify-center flex-shrink-0">
                    <Lock className="w-3.5 h-3.5 text-white" />
                  </span>
                  <span className="text-left">
                    <p className="text-sm font-medium text-gray-800">Schedule Rules</p>
                    <p className="text-xs text-gray-400">Non-negotiable — AI always follows these</p>
                  </span>
                </span>
              ...
              {/* One-off Events — coming soon */}
              <div className="w-full flex items-center justify-between px-4 py-3.5 rounded-xl bg-gray-50 border border-gray-100 opacity-50">
                <span className="flex items-center gap-3">
                  <span className="w-8 h-8 rounded-lg bg-gray-200 flex items-center justify-center flex-shrink-0">
                    <Zap className="w-3.5 h-3.5 text-gray-500" />
                  </span>
                  <span className="text-left">
                    <p className="text-sm font-medium text-gray-800">One-off Events</p>
                    <p className="text-xs text-gray-400">Adjust for a specific date</p>
                  </span>
                </span>
                <span className="text-xs font-medium text-gray-400 bg-gray-200 px-2 py-0.5 rounded-full">Soon</span>
              </div>
              ...
              >
                <span className="flex items-center gap-3">
                  ...
                  <span className="text-left">
                    <p className="text-sm font-medium text-gray-800">General Preferences</p>
                    <p className="text-xs text-gray-400">Meals, timing, cleaning & more</p>
                  </span>
                </span>
                <span className="flex items-center gap-2 flex-shrink-0">
                  {changedCount > 0 && (
            ...
            {/* Rules diff review */}
            {(() => {
              const appliedIds = new Set(appliedRulesSnapshot.map((r) => r.id));
              const currentIds = new Set(rules.map((r) => r._id as string));
              const added    = rules.filter((r) => !appliedIds.has(r._id as string));
              const unchanged = rules.filter((r) => appliedIds.has(r._id as string));
              const removed  = appliedRulesSnapshot.filter((r) => !currentIds.has(r.id));
              const hasChanges = added.length > 0 || removed.length > 0;
              const hasAny = rules.length > 0 || removed.length > 0;

              if (!hasAny) return null;
              return (
                <div className="mt-5">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-widest mb-2">
                    Schedule Rules · what changes
                  </p>
                  <div className="flex flex-col gap-1.5 mb-4">
                    {added.map((r) => (
                      <div key={r._id} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-green-50 border border-green-100">
                        <span className="text-xs font-bold text-green-500 w-3 flex-shrink-0">+</span>
                        <p className="text-xs text-green-800 truncate">{r.title}</p>
                        <span className="ml-auto text-xs text-green-500 flex-shrink-0">New</span>
                      </div>
                    ))}
                    {removed.map((r) => (
                      <div key={r.id} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-red-50 border border-red-100">
                        <span className="text-xs font-bold text-red-400 w-3 flex-shrink-0">−</span>
                        <p className="text-xs text-red-700 truncate line-through">{r.title}</p>
                        <span className="ml-auto text-xs text-red-400 flex-shrink-0">Removed</span>
                      </div>
                    ))}
                    {unchanged.map((r) => (
                      <div key={r._id} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-gray-50 border border-gray-100">
                        <Lock className="w-3 h-3 text-gray-300 flex-shrink-0" />
                        <p className="text-xs text-gray-400 truncate">{r.title}</p>
                        <span className="ml-auto text-xs text-gray-300 flex-shrink-0">Active</span>
                      </div>
                    ))}
                  </div>
                  {hasChanges && (
                    <button
                      ...
                      className="w-full py-3.5 rounded-xl bg-gray-900 text-white text-sm font-semibold hover:opacity-90 transition-opacity disabled:opacity-60 flex items-center justify-center gap-2"
                    >
                      {isRegenerating
        ...
        {view === "preferences" && (
          <>
            <div className="flex items-center gap-2 mb-4">
              <button onClick={() => setView("hub")} className="p-1 -ml-1 text-gray-400 hover:text-gray-700">
                <ChevronLeft className="w-5 h-5" />
              </button>
              <p className="text-sm font-semibold text-gray-900">General Preferences</p>
            </div>
      ...
      {/* Sticky regenerate button — only in preferences view */}
      <div className={cn(
        "fixed bottom-0 left-0 right-0 px-4 pb-6 pt-3 bg-white border-t border-gray-100 transition-all duration-200",
        (view !== "preferences" || changedCount === 0) ? "opacity-0 pointer-events-none translate-y-2" : "opacity-100 translate-y-0"
      )}>
        <button
          onClick={handleRegenerate}
          disabled={isRegenerating}

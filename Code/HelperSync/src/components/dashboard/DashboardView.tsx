...
export function DashboardView({ householdId }: DashboardViewProps) {
  ...
  const timetable = useQuery(api.timetable.getTimetable, { householdId });
  const taskLogs = useQuery(api.taskLogs.getLogsForDate, { householdId, date: todayStr });
  const medLogs = useQuery(api.medicationLogs.getLogsForDate, { householdId, date: todayStr });

  const elderlyMembers = household?.members.filter((m) => m.role === "Elderly" && m.medications?.trim()) ?? [];
  const todayTasks = timetable?.weeklyTasks.find((d) => d.day === todayDay)?.tasks ?? [];
